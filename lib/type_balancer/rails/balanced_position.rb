...
module TypeBalancer
  module Rails
    # Stores and manages balanced positions for records
    class BalancedPosition < ActiveRecord::Base
      ...
      validates :record_id, uniqueness: { scope: [:record_type, :cache_key] }
      ...
      def self.table_definition
        proc do |t|
          ...
          t.index [:record_type, :record_id, :cache_key], unique: true, name: "index_type_balancer_positions_on_record_and_cache"
        end
      end
    end
  end
